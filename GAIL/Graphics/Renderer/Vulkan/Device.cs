...
namespace GAIL.Graphics.Renderer.Vulkan
{
	...
	public class Device : IDisposable {
		...
		public Device(Renderer renderer) {
			...
			renderer.Logger.LogDebug("Searching for suitable Vulkan physical device.");
			...
			if (!oneDeviceSuitable) {
				renderer.Logger.LogFatal("Vulkan: No suitable physical device found.");
				throw new APIBackendException("Vulkan", "No suitable physical device found");
			}
		}
		private bool CreateLogicalDevice() {
			renderer.Logger.LogDebug("Creating Vulkan logical device.");
			...
			unsafe {
				...
				bool succeeded = Utils.Check(API.Vk.CreateDevice(physicalDevice, in createInfo, Allocator.allocatorPtr, out logicalDevice), renderer.Logger, "Failed to create logical device", false);
		...
		public PhysicalDevice[] GetPhysicalDevices(Instance instance) {
			...
			}, out PhysicalDevice[] devices, renderer.Logger, "PhysicalDevices", true);

			if (devices.Length == 0) {
				renderer.Logger.LogFatal("Vulkan: Failed to find GPU with Vulkan support!");
		...
		public Swapchain.SupportDetails CheckSwapChainSupport(PhysicalDevice device) {
			...
			unsafe {
				_ = Utils.Check(renderer.surface.Extension.GetPhysicalDeviceSurfaceCapabilities(device, renderer.surface.surface, out details.Capabilities), renderer.Logger, "Failed at getting PhysicalDevice surface capabilities", true);

				// Surface formats
				Utils.GetArray((Pointer<SurfaceFormatKHR> pointer, Pointer<uint> count) => {
					return renderer.surface.Extension.GetPhysicalDeviceSurfaceFormats(device, renderer.surface.surface, count, pointer);
				}, out details.Formats, renderer.Logger, "PhysicalDeviceSurfaceFormats", true);

				// Surface present modes
				Utils.GetArray((Pointer<PresentModeKHR> pointer, Pointer<uint> count) => {
					return renderer.surface.Extension.GetPhysicalDeviceSurfacePresentModes(device, renderer.surface.surface, count, pointer);
				}, out details.PresentModes, renderer.Logger, "PhysicalDeviceSurfacePresentModes", true);
			}
		...
		public bool CheckDeviceExtensionsSupport(PhysicalDevice device) {
			...
			}, out ExtensionProperties[] availableExtensions, renderer.Logger, "DeviceExtensionProperties", false)) {
				return false;
			}
		...
		public QueueFamilyIndices FindQueueFamilies(PhysicalDevice device) {
			...
			unsafe {
				...
				}, out QueueFamilyProperties[] queueFamilies, renderer.Logger, "PhysicalDeviceQueueFamilyProperties", true);
				...
				for (uint i = 0; i < queueFamilies.Length; i++)
				{
					...
					_ = Utils.Check(renderer.surface.Extension.GetPhysicalDeviceSurfaceSupport(device, i, renderer.surface.surface, out Bool32 presentSupport), renderer.Logger, "Unable get physical device surface support", true);
		...
		public bool WaitIdle() {
			return Utils.Check(API.Vk.DeviceWaitIdle(logicalDevice), renderer.Logger, "The device failed to wait idle");
		}
