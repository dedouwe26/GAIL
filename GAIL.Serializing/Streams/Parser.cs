...
public class Parser : IDisposable {
	...
	public virtual T ReadSerializable<T>(ISerializable.Info info, IFormatter? formatter = null) where T : ISerializable {
		ISerializable s = ReadSerializable(info, formatter);
		if (s is T serializable) {
			return serializable;
		}
		throw new InvalidCastException($"The serializable ({s.GetType()}) is not of type {typeof(T)}");
	}

	/// <summary>
	/// Reads a serializable and returns the value of the serializable.
	/// </summary>
	/// <typeparam name="T">The type of the value of the serializable.</typeparam>
	/// <param name="info">The info of that serializable.</param>
	/// <param name="formatter">The formatter to use.</param>
	/// <exception cref="InvalidCastException">The serializable is not of type <see cref="ISerializable{T}"/>.</exception>
	/// <returns>The value of the parsed serializable.</returns>
	public T ReadValue<T>(ISerializable.Info info, IFormatter? formatter = null) {
		return ReadSerializable<ISerializable<T>>(info, formatter).Value;
	}
	...
	public virtual uint ReadUInt(IFormatter? formatter = null) {
		return ReadValue<uint>(UIntSerializable.Info, formatter);
	}
	/// <summary>
	...
	public virtual int ReadInt(IFormatter? formatter = null) {
		return ReadValue<int>(IntSerializable.Info, formatter);
	}
	/// <summary>
	...
	public virtual byte ReadByte(IFormatter? formatter = null) {
		return ReadValue<byte>(ByteSerializable.Info, formatter);
	}
	/// <summary>
	...
	public virtual string ReadString(IFormatter? formatter = null) {
		return ReadValue<string>(StringSerializable.Info, formatter);
	}
